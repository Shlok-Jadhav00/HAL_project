...
for dataset_name in datasets:
    data_path = os.path.join(r'e:\AEIA', 'sample_data', dataset_name)
    output_path = os.path.join(output_dir, dataset_name.replace('.csv', '_report.pdf'))
    
    print(f'Processing {dataset_name}...')
    
    # 1. Load Data
    df, col_types, _ = data_loader.load_dataset(data_path)
    di = {'filename': dataset_name, 'row_count': len(df), 'column_count': len(df.columns), 'import_date': '2026-07-13'}
